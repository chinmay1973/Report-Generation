...
def report_generation(BatchCode):
    global running
    while running:
        if not running:
            break
        ...
        WHERE id IN (SELECT student_id FROM test.class_registration 
        WHERE scheme_id IN (SELECT id FROM test.scheme 
        WHERE term_id=90) AND session_id = {user_input[0]})) AS session_4th_yr 
            ...
            studentSubject.student_id IN (
                ...
                WHERE id IN (
                    ...
                    WHERE scheme_id IN (
                        ...
                        FROM test.scheme 
                        WHERE term_id = 90 
                        AND session_id = {user_input[0]}
                    )
                    AND session_id > {int(user_input[0]) - 8}
                )
            )
    ) us
JOIN 
...
ORDER BY 
    ...
    us.subject_id ASC;
        ...
        query_student_id = (f"""SELECT id, enroll_no FROM test.student
                            WHERE id IN (SELECT student_id FROM test.class_registration 
                            WHERE scheme_id IN (SELECT id FROM test.scheme
                            WHERE term_id=90) AND session_id = {user_input[0]})
                            ORDER BY id ASC;""")
        ...
        for session_value in user_input:
            ...
            final_file = f'Batch{year_value}Students.docx'
            # Use the copy_layout function
            newDoc = docx.Document()
            newDoc.save(f'Batch{year_value}Students.docx')
            newdoc = f'Batch{year_value}Students.docx'
            ...
            for student_id, sessions in subjects_by_student.items():
                ...
                for session_id, subjects in sessions.items():
                    ...
                    WHERE id IN (SELECT student_id FROM test.class_registration 
                    WHERE scheme_id IN (SELECT id FROM test.scheme 
                    WHERE term_id=90) AND session_id > {int(user_input[0])-8}))
                    ORDER BY student_id ASC, session_id ASC, subject_id ASC;
                    """)
                    mycursor.execute(query_grade_result)
                    grade_result = mycursor.fetchall()
            ...
            shutil.move(final_file, f'../../Reports/YearWise/{final_file}')
...
label_head = ttk.Label(root, text="YEAR  WISE  REPORT  GENERATION")
...
for year in years:
    ...
    if year_value >= 2015:
        mapped_value = 15 + 2 * (year_value - 2015)
        batch_details.insert("", "end", values=(year_value, mapped_value))
